...
public class PlayerConnection implements PacketPlayInListener {
    ...
    public void a(PacketPlayInFlying packetplayinflying) {
        ...
        if (!this.player.viewingCredits) {
            ...
            if (packetplayinflying.hasPos && !(packetplayinflying.y == -999.0D && packetplayinflying.stance == -999.0D)) {
            ...
            if (this.checkMovement && !this.player.dead) {
                ...
                if (!this.minecraftServer.getAllowFlight() && !this.player.abilities.canFly && !worldserver.c(axisalignedbb)) { // CraftBukkit - check abilities instead of creative mode
                    if (d11 >= -0.03125D) {
                        ++this.f;
                        if (this.f > 80) {
                            if (lastKickedForFly + TimeUnit.MINUTES.toMillis(1) < networkManager.currentTime) { // Guardian - Only kick them if we are currently doing checks, add a delay between kicks
